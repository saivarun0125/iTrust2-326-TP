...
public class APIVaccineOfficeVisitTest {
    ...
    public void testPreScheduledVaccineOfficeVisit () throws Exception {
        ...
        final MockHttpServletRequestBuilder post = post( "/api/v1/vaccineofficevisits" );
        post.contentType( MediaType.APPLICATION_JSON ).content( TestUtils.asJsonString( visit ) );

        mvc.perform( post ).andExpect( status().isOk() );

        // mvc.perform( post( "/api/v1/vaccineofficevisits" ).contentType(
    ...
    // Assert.assertEquals( 1, vaccineOfficeVisitService.count() );
    //
    // mvc.perform( get( "/api/v1/VaccineOfficeVisits" ) ).andExpect(
    // status().isOk() )
    // .andExpect( content().contentType( MediaType.APPLICATION_JSON_VALUE ) );
    //
    // /* Test getForHCP and getForHCPAndPatient */
    // VaccineOfficeVisit v = vaccineOfficeVisitService.build( visit );
    // List<VaccineOfficeVisit> vList = vaccineOfficeVisitService.findByHcp(
    // v.getHcp() );
    // assertEquals( vList.get( 0 ).getHcp(), v.getHcp() );
    // vList = vaccineOfficeVisitService.findByHcpAndPatient( v.getHcp(),
    // v.getPatient() );
    // assertEquals( vList.get( 0 ).getHcp(), v.getHcp() );
    ...
    // v = vaccineOfficeVisitService.build( visit );
    //
    ...
    // assertEquals( HouseholdSmokingStatus.INDOOR,
    // visit.getHouseSmokingStatus() );
    // assertEquals( Integer.valueOf( 30 ), visit.getLdl() );
    // assertEquals( PatientSmokingStatus.FORMER,
    // visit.getPatientSmokingStatus() );
    ...
    // */
    // final Long id = vaccineOfficeVisitService.findByPatient(
    // userService.findByName( "patient" ) ).get( 0 ).getId();
    //
    ...
    // status().isConflict() );
    //
    // mvc.perform( get( "/api/v1/VaccineOfficeVisits/" + id ) ).andExpect(
    // status().isOk() )
    // .andExpect( content().contentType( MediaType.APPLICATION_JSON_VALUE ) );
    //
    ...
    // visit.setId( "101" );
    // mvc.perform( put( "/api/v1/VaccineOfficeVisits/" + tempId ).contentType(
    // MediaType.APPLICATION_JSON )
    // .content( TestUtils.asJsonString( visit ) ) ).andExpect(
    // status().isNotFound() );
    //
